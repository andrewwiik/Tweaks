...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext fromView:(UIView *)fromView toView:(UIView *)toView
{
    ...
    if ([fromView respondsToSelector:@selector(delegate)]) {
        if ([fromView.delegate isKindOfClass:NSClassFromString(@"CCXMainControlsPageViewController")]) {
            updateMaskingStuffFrom = YES;
            ((CCXMainControlsPageViewController *)fromView.delegate).animationProgressValue = 0;
        }
    }

    if ([toView respondsToSelector:@selector(delegate)]) {
        if ([toView.delegate isKindOfClass:NSClassFromString(@"CCXMainControlsPageViewController")]) {
            updateMaskingStuffTo = YES;
            ((CCXMainControlsPageViewController *)toView.delegate).animationProgressValue = 0;
        }
    }
