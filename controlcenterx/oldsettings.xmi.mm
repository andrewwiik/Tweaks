...
- (void)viewDidLoad
{
    ...
    self.books[0] = [NSMutableArray new];
    self.books[1] = [NSMutableArray new];
    for (int x = 0; x < 5; x++) {
    	self.books[0][x] = @{@"label": [[NSString alloc] initWithFormat:@"%i番目の項目", x], @"tag": [NSNumber numberWithInt:x]};
    }
    for (int y = 0; y < 2; y++) {
    	self.books[1][y] = @{@"label": [[NSString alloc] initWithFormat:@"%i番目の項目", y], @"tag": [NSNumber numberWithInt:y]};
    }
 //    self.books[0][0] = [NSMutableDictionary new]
 //    for (int y = 0; y < 2; y++) {
 //    	self.books[y] = [NSMutableArray new];
 //    	for (int i = 0; i < 20; i++) {
 //        	NSDictionary *dc = @{@"label": [[NSString alloc] initWithFormat:@"%i番目の項目", i], @"tag": [NSNumber numberWithInt:i]};
 //        	[(NSMutableArray *)self.books[y] addObject:dc];
 //    	}
 //    // [self.tableView setSeparatorColor:[UIColor colorWithWhite:1 alpha:0.3]];
	// }

	if (NSClassFromString(@"FSSwitchPanel")) {
		self.usingFlipControlCenter = YES;
		self.templateBundle = [[NSBundle alloc] initWithPath:@"/Library/Application Support/FlipControlCenter/TopShelf8.bundle"];
		self.settingsFile = @"/var/mobile/Library/Preferences/com.rpetrich.flipcontrolcenter.plist";
		self.preferencesApplicationID = @"com.rpetrich.flipcontrolcenter";
		self.notificationName = @"com.rpetrich.flipcontrolcenter.settingschanged";
		NSDictionary *settings = nil;
		self.enabledKey = @"EnabledIdentifiers";
		self.disabledKey = @"DisabledIdentifiers";
		...
		// NSMutableArray *arrayToAddNewIdentifiers = [[specifier propertyForKey:@"flipswitchNewAreDisabled"] boolValue] ? self.disabledIdentifiers : self.enabledIdentifiers;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.usingFlipControlCenter) {
    	...
    	backgroundView.backgroundColor = [panel primaryColorForSwitchIdentifier:switchIdentifier] ? [panel primaryColorForSwitchIdentifier:switchIdentifier] : [UIColor grayColor];
