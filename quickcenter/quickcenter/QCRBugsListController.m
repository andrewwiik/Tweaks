...
- (NSArray *)specifiers {
  ...
  [specifier setProperty:[UIImage imageWithContentsOfFile:@"/Library/PreferenceBundles/quickcenter.bundle/bugProduct.png"] forKey:@"ACUIAppInstallIcon"];
  ...
  [crashesLabel setProperty:[NSString stringWithFormat:@"Listed above are all the crashes that were possibly caused by QuickCenter, you may select a crash to submit a bug report to us."] forKey:@"footerText"];
...
- (void)loadCrashReports {
  ...
  NSPredicate *quickcenterFilter = [NSPredicate predicateWithFormat:@"self CONTAINS 'QuickCenter'"];
  ...
  for (NSString *path in onlySpringBoard) {
    ...
    if (crash.isSymbolicated) {
      ...
      if (blames) {
        NSMutableArray *blamesFiltered = [[blames filteredArrayUsingPredicate:quickcenterFilter] mutableCopy];
