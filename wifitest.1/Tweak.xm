...
NSDictionary *settings = [NSDictionary dictionaryWithContentsOfFile:@"/User/Library/Preferences/com.atwiiks.conditionalwifi.plist"];
   NSLog(@"SETTINGS LOAD: %@",settings);
internet = YES;
...
//         PSSpecifier *specifier = [%c(PSSpecifier) preferenceSpecifierNamed:[applications objectForKey:key] target:self set:@selector(setWifiUsageValue:forSpecifier:) get:@selector(wifiUsageValueForSpecifier:) detail:nil cell:PSSwitchCell edit:nil];
...
//   if ([[NSBundle mainBundle].bundleIdentifier isEqualToString:@"com.spotify.client"]) { // Viber App
   NSDictionary *settings = [NSDictionary dictionaryWithContentsOfFile:@"/User/Library/Preferences/com.atwiiks.conditionalwifi.plist"];
   NSLog(@"SETTINGS LOAD: %@",settings);
   if ([[settings objectForKey:[NSBundle mainBundle].bundleIdentifier] isEqual:@NO]) {
